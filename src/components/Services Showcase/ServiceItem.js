...
const ServiceItem = ({ img, name, description, price , type}) => {
    ...
    return (
        <div className="bg-white border border-gray-100 transition transform duration-700 hover:shadow-xl hover:scale-105 p-4 rounded-lg relative">
            ...
            <div className="flex flex-col items-center my-3 space-y-2">
                ...
                <p className="text-gray-500 poppins text-sm text-center">{description.slice(0,100)}</p>
                <h2 className="text-gray-900 poppins text-2xl font-bold">${price}</h2>
                <button className="bg-primary text-white px-8 py-2 focus:outline-none poppins rounded-full mt-24 transform transition duration-300 hover:scale-105" onClick={handleRoute}>Appoinment Please</button>
            </div>
        </div>
    )
}
